...
class GeminiTranscriptionService(private val context: Context) {
    ...
    companion object {
        ...
        private val MODELS_TO_TRY = listOf("gemini-3.5-flash", "gemini-2.5-flash", "gemini-flash-latest")
    }
    ...
    suspend fun transcribeAudioFile(audioFile: File): Result<String> = withContext(Dispatchers.IO) {
        ...
        val apiKey = when {
            ...
            else -> "AQ.Ab8RN6IJzWO6vgtMM1_9a0Qb7tvaK7_hgjuGkRLYN91Xh9ZGHA"
        }
        ...
        for (modelName in MODELS_TO_TRY) {
            try {
                Log.d(TAG, "Attempting transcription using model: $modelName for file: ${audioFile.absolutePath}, size: ${audioFile.length()} bytes")
                ...
                Log.e(TAG, "Transcription failed with exception on model $modelName", e)
                lastException = e
            }
        }
